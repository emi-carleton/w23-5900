...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        landmark = Landmark(
            name="Mountain", collide=True, shape=Box(width=5,length=8)
        )
        world.add_landmark(landmark)
    ...
    def reset_world_at(self, env_index: int = None):
        ...
        #Setting Mountain position (15,0)
        if env_index is not None:
            ...
            x = torch.tensor([[15.0,0.0]],device=self.world.device, dtype=torch.float32,)
            repeatX = x.repeat(self.world.batch_dim,1)
            self.world.landmarks[2].set_pos(
                repeatX,
                batch_index=env_index,
            )
        print("Mountain position: ", self.world.landmarks[2].state.pos)
    ...
    def observation(self, agent):
        ...
        if not agent.movable:
            ...
            plane_pos= plane_pos = self.world.agents[0].state.pos #important for scenario 2
            

        #if a tower has detected anomaly in another tower's communication (there is no communication)
        if(tower_plane_distance.nelement == 0):
            #supposed to be tower 1's responsibility. but tower 2 will take over
            if(self.world.agents[0].state.pos < 15):
                #calculate Θ
                dx = self.world.comm[-1].plane_pos.x - self.world.agents[0].state.pos.x
                dy = self.world.comm[-1].plane_pos.y - self.world.agents[0].state.pos.y
                theta = math.degrees(math.atan2(dy, dx))
                #calculate distance from plane to tower 2 using cosine rule
                a = 10
                b = self.world.comm[-1].tower_plane_distance
                tower_plane_distance = math.sqrt(a**2 + b**2 - 2 * a * b * math.cos(math.radians(theta)))#cosine rule 

            #supposed to be tower 2's responsibility. but tower 1 will take over
            else:
                #calculate Θ
                dx = self.world.comm[-1].plane_pos.x - self.world.agents[1].state.pos.x
                dy = self.world.comm[-1].plane_pos.y - self.world.agents[1].state.pos.y
                theta = math.degrees(math.atan2(dy, dx))
                #calculate distance from plane to tower 1 using cosine rule
                a = 10
                b = self.world.comm[-1].tower_plane_distance
                tower_plane_distance = math.sqrt(a**2 + b**2 - 2 * a * b * math.cos(math.radians(theta)))#cosine rule 
    ...
    def reward(self, agent: Agent):
        ...
        # reward for staying in the arc of tower 1 
        if(self.world.agents[0].state.pos < 15):
            self.rew += torch.sqrt(
                        torch.sum(torch.square(self.world.agents[0].state.pos-self.world.agents[1].state.pos),dim=-1))
        # reward for staying in the arc of tower 2 
        else:
            self.rew += torch.sqrt(
